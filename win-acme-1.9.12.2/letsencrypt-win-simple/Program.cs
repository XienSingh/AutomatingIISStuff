...
namespace PKISharp.WACS
{
    internal partial class Program
    {
        ...
        private static string SiteOptions ="";
        ...
        private static void CreateNewCertificate(RunLevel runLevel, int type = 0)
        {
            _log.Information(true, "Running in {runLevel} mode", runLevel);
            var tempRenewal = CreateRenewal(_options);
            using (var scope = AutofacBuilder.Renewal(_container, tempRenewal, runLevel,type))
            {
                // Choose target plugin
                ...
                var targetPlugin = scope.Resolve<ITargetPlugin>(new NamedParameter("Binding", BindingsToAdd));
     
                var target = runLevel == RunLevel.Unattended ? targetPlugin.Default(_optionsService) : targetPlugin.Aquire(_optionsService, _input, runLevel,BindingsToAdd);
                ...
                try
                {
                    ...
                    foreach (var installFactory in installFactories)
                    {
                        ...
                        {
                            installFactory.Aquire(tempRenewal, _optionsService, _input, runLevel,siteID);
                        }
                    }
