...
namespace PKISharp.WACS.Plugins.TargetPlugins
{
    ...
    internal class Manual : ITargetPlugin
    {
        ...
        Target ITargetPlugin.Aquire(IOptionsService optionsService, IInputService inputService, RunLevel runLevel, string BindingsToAdd)
        {
            var input ="";
            if (BindingsToAdd != "")
            {
            ...
            else{input = inputService.RequestString("Enter comma-separated list of host names, starting with the primary one"); }
            var target = Create(input);
